...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        ...
        sptw::SPTW_ERROR err = sptw::create_tiled_raster(raster_fn, RASTER_SIZE, RASTER_SIZE, 1,
    ...
    int tiles_per_rank = (RASTER_SIZE*RASTER_SIZE)/(TILE_SIZE*TILE_SIZE*process_count); 
    ...
    for(int i = 0; i < tiles_per_rank; i++)
    {
        ...
        int x = tile % (RASTER_SIZE / TILE_SIZE);
        int y = tile / (RASTER_SIZE / TILE_SIZE);
